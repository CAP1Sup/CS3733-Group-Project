...
interface GenerateShowReportRequest {
    email: string;
    passwd: string;
    venue: string;
    show: string;
...
export const lambdaHandler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    // Parse the request
    const request: GenerateShowReportRequest = JSON.parse(event.body);
    ...
    try {
        ...
        if (venues.some((venue) => venue.name === request.venue)) {
            ...
            if (await venueExists(request.venue, db)) {
                throw "You're not authorized to view reports about that venue";
            }
